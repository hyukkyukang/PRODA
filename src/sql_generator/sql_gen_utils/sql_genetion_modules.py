...
def select_generator(
    ...
    if num_select == 0:
        ...
        agg = "NONE"
        if first_table not in PRIMARY_KEYS.keys():
            col = "*"
            col_rep = "*"
        else:
    ...
    while not done:
        ...
        if nesting_position == 0:
            ...
            if inner_query_obj is None:
                inner_query, inner_query_result, _ = inner_query_generator(
                    args,
                    df,
                    n,
                    rng,
...
def inner_query_generator(
    ...
    if correlation_column is not None:
        if not use_agg_sel:
            ...
            if len(agg_cols) == 1 and agg_cols[0][1] == "*":
                select_columns = [correlation_column] + [
                    table_column for table_column in table_columns if table_column.split(".")[0] in necessary_tables
                ]
            new_result_tuples = []
