...
from src.test.utils import query_graph_to_generic_templates
...
class TMT:
    ...
    def _template_selection(self, G, I, all_templates):
        ...
        for tg_idx in range(M.shape[1]):
            ...
            if sum(M[:, tg_idx]) == len(template.graph.edges):
                ...
                candidate_solutions.append(
                    (
                        set([template]),
                        set([(src.signature, dst.signature) for src, dst in template.graph.edges]),
                        set(template.reference_points),
                    )
                )
        ...
        while candidate_solutions:
            ...
            print(
                ...
                len(composeable_templates),
                len(template_set),
                len(edge_set),
                len(reference_point_set),
            )

            templates_to_add = filter(
                lambda t: t not in template_set and reference_point_set.intersection(set(t.reference_points)),
                composeable_templates,
            )
            for template in templates_to_add:
                new_template_set = set([template]).union(template_set)
                new_edge_set = edge_set.union(
                    set([(src.signature, dst.signature) for src, dst in template.graph.edges])
                )
            ...
            if not best_solution:
                ...
                new_solution_uses_less_template = (len(best_edge_set) == len(edge_set)) and len(
                    best_template_set
                ) > len(template_set)
    ...
    def _call(self, G, templates):
        ...
        def categorize_templates_by_edges(templates):
            ...
            for template in templates:
                edges = [
                    template.graph.edges[src, dst]["data"]
                    for src, dst in template.graph.all_edges_of(template.root_relation)
                ]
        ...
        def compose_templates(cur_template_group, group_of_templates):
            ...
            membership_templates, selection_templates, remaining_templates = categorize_templates_by_edges(
                templates_in_group
            )
            ...
            phrase_str = compose_clauses(
                group_subject_relation.label, projection_clause_str, selection_clause_str, other_clause_str
            )
            return phrase_str
        ...
        while root_relations:
            ...
            for selected_template in filter(
                lambda t: any(r in leaves for r in t.graph.relations), sorted_composeable_template
            ):
