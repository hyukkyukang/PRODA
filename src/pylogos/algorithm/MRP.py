...
class MRP:
    ...
    def _call(self, current_node, parent_node, previous_reference_point, query_graph, self_path=None):
        ...
        if current_node in query_graph.reference_points:
            ...
            while self_path:
                ...
                string_builder.add_join_conditions(
                    previous_reference_point, current_node, edge_desc, dst_node, has_membership
                )
        ...
        while opened:
            ...
            string_builder.add(
                self._call(pop_current_node, pop_parent_node, pop_referece_point, query_graph, self_path)
            )

        return string_builder

    def _call_2(self, current_node, parent_node, previous_reference_point, query_graph, self_path=None, opened=None):
        self_path = [] if self_path == None else self_path
        ...
        if current_node in query_graph.reference_points:
            ...
            while self_path:
                ...
                if query_graph.has_edge(src_node, dst_node):
                    ...
                    edge_desc = ""
                    # edge_desc = self.label_edge(query_graph, dst_node, src_node)
        ...
        if len(opened) > 0:
            pop_current_node, pop_parent_node, pop_referece_point = opened.pop(-1)
            string_builder.add(
                self._call_2(pop_current_node, pop_parent_node, pop_referece_point, query_graph, self_path, opened)
            )
        ...
        for att in graph.get_out_going_nodes(relation):
            ...
            if type(out_edge_from_relation) == Selection:
                    ...
                    if type(out_edge_from_att) == Predicate:
                        ...
                        if type(dst) == Value:
                            ...
                            associated_relations = list(
                                filter(lambda n: type(n) == Relation, graph.get_out_going_nodes(dst))
                            )
                            assert (
                                len(associated_relations) == 1
                            ), f"Unexpected number of parent relations, {len(associated_relations)} "
                            ...
                            associated_relations = [
                                n for n in graph.get_out_going_nodes(next_att_node) if type(n) == Relation
                            ]
                            assert (
                                len(associated_relations) == 1
                            ), f"Unexpected number of parent relations, {len(associated_relations)} "
                ...
                while type(selected_edge) == Grouping and type(selected_node) == Attribute:
                    ...
                    assert (
                        len(next_nodes) < 2
                    ), f"Expected only one outgoing node connnected to grouping attribute, but found {len(next_nodes)} "
                    if not next_nodes:
                        break
                ...
                assert (
                    len(out_nodes) == 1
                ), f"Having condition must be connected to only one node, but found {len(out_nodes)} "
                assert (
                    type(out_nodes[0]) == Value
                ), f"Having condition must be connected to value node, but found {type(out_nodes[0])} "
                value_node = out_nodes[0]

                # Check edge type
                out_edge = graph.get_edge(function_node, value_node)
                assert (
                    type(out_edge) == Predicate
                ), f"Having attribute must be connected to value node through Predicate edge, but found {type(out_edge)} "
