...
from src.pylogos.query_graph.queryGraph import Node, Edge, Operation, Ordering, Relation, Attribute, Function, Value
from src.pylogos.query_graph.queryGraph import Projection, Selection, Join, Grouping, Having, Limit
from src.pylogos.query_graph.queryGraph import Aggregation, OperatorType, Query_graph
...
class Template:
    def __init__(self, raw_generic_templates, raw_specific_templates):
        ...
        self.generic_templates = [
            Generic_template(raw_generic_template) for raw_generic_template in raw_generic_templates
        ]
        self.specific_templates = [
            Specific_template(raw_specific_template) for raw_specific_template in raw_specific_templates
        ]
...
class Template_instance(metaclass=ABCMeta):
    ...
    def add_edge_to_query_graph_path(template, query_graph):
        template.query_graph_path = [
            (src, dst, query_graph.edges[src, dst]["data"]) for src, dst in template.query_graph_path
        ]
        return template
    ...
    def _find_matching_idx_in_query_graph_path(self, template_path_item, query_graph_path, star_idx=0):
        ...
        for idx in range(star_idx, len(query_graph_path)):
            ...
            is_matched = (
                inspect.isclass(target, template_path_item)
                if inspect.isclass(template_path_item)
                else target.name == template_path_item.name
            )
            if is_matched:
                break
    ...
    def create_template_to_query_graph_idx_mapping(self):
        ...
        def move_tp_idx(tp_idx):
            tp_idx += 1
            while (
                tp_idx < len(self.template_path)
                and not inspect.isclass(self.template_path[tp_idx])
                and self.template_path[tp_idx].name == "*"
            ):
                tp_idx += 1
            return tp_idx
    ...
    def do_starts_with_edge_type(self, edge_to_compare):
        assert (
            len(self.query_graph_path[0]) == 3
        ), "Error on checking the starting edge: Need to call add_edge_to_query_graph_path first!"
...
def get_isomorphic_subgraph_paths(graph, target_sub_graph, node_matcher, edge_matcher):
    digraph_matcher = nx.algorithms.isomorphism.vf2userfunc.DiGraphMatcher(
        graph, target_sub_graph, node_match=node_matcher, edge_match=edge_matcher
    )
    subgraph_paths = []
    if digraph_matcher.subgraph_is_isomorphic():
        for subgraph in [
            _ for _ in digraph_matcher.subgraph_isomorphisms_iter()
        ]:  # Not sure if it is each subgraphs or each paths
            subgraph_path = []
...
def is_connectable(template1, template2):
    return (
        Node.is_equivalent(template1[0], template2[0])
        or Node.is_equivalent(template1[-1], template2[0])
        or Node.is_equivalent(template2[-1], template1[0])
    )
...
def get_matched_generic_templates(query_graph, template):
    def create_dummy_edge_instance(edge_object, src_node, dst_node):
        return (
            edge_object(OperatorType.Equal, src_node, dst_node)
            if edge_object == Operation
            else edge_object(src_node, dst_node)
        )
    ...
    for idx in range(0, len(template_path) - 2, 2):
        ...
        assert (
            issubclass(src, Node) and issubclass(dst, Node) and issubclass(edge, Edge)
        ), "Error when constructing graph from generic template"
    ...
    # Main logic
    digraph_matcher = nx.algorithms.isomorphism.vf2userfunc.DiGraphMatcher(
        query_graph, graph_to_compare, node_match=node_matcher, edge_match=edge_matcher
    )
    matched_templates = []
    if digraph_matcher.subgraph_is_isomorphic():
...
def is_connectable(selected_templates, testing_template):
    ...
    for selected_template in selected_templates:
        ...
        if (
            Node.is_equivalent(get_src_of_path(selected_query_graph_path), get_dst_of_path(testing_template))
            or Node.is_equivalent(get_src_of_path(testing_template), get_dst_of_path(selected_query_graph_path))
            or Node.is_equivalent(get_src_of_path(selected_query_graph_path), get_src_of_path(testing_template))
        ):
            return True
    return False
...
def template_selection(ori_query_graph, ori_templates):
    ...
    while len(query_graph):
        ...
        specific_templates_filtered = list(
            filter(lambda k: get_matched_specific_templates(query_graph, k), templates["specific"])
        )
        # Filter templates condition 2: one repeated node from collected templates
        specific_template_filtered = list(
            filter(lambda k: is_connectable(selected_templates, k.query_graph_path), specific_templates_filtered)
        )
        ...
        if specific_template_filtered:
            ...
            # Filter templates condition 1: that covers graph
            generic_templates_filtered = flatten(
                map(lambda k: get_matched_generic_templates(query_graph, k), templates["generic"])
            )
            # Filter templates condition 2: one repeated node from collected templates
            generic_template_filtered = list(
                filter(lambda k: is_connectable(selected_templates, k.query_graph_path), generic_templates_filtered)
            )
            # Sort by coverage score
...
def create_template_dic(root, remaining_templates):
    ...
    def combine_template_dic(td1, td2):
        ...
        return {
            key: td1.get(key, []) + td2.get(key, []) for key in set([k for k in td1.keys()] + [k for k in td2.keys()])
        }
    ...
    def templates_for_given_root_and_edge_type(templates, root, edge_type):
        ...
        selected_templates = list(
            filter(lambda t: t.do_starts_with_node(root) and t.do_starts_with_edge_type(edge_type), templates)
        )
        # Remove templates
    ...
    def for_projection():
        return {
            "projection": [
                {"template": t, "nested": {}}
                for t in templates_for_given_root_and_edge_type(remaining_templates, root, Projection)
            ]
        }
    ...
    def for_ordering():
        return {
            "ordering": [
                {"template": t, "nested": {}}
                for t in templates_for_given_root_and_edge_type(remaining_templates, root, Ordering)
            ]
        }
    ...
    def for_grouping():
        return {
            "grouping": [
                {"template": t, "nested": {}}
                for t in templates_for_given_root_and_edge_type(remaining_templates, root, Grouping)
            ]
        }
    ...
    def for_limit():
        return {
            "limit": [
                {"template": t, "nested": {}}
                for t in templates_for_given_root_and_edge_type(remaining_templates, root, Limit)
            ]
        }
    ...
    tmp = (
        reduce(
            combine_template_dic,
            [for_projection(), for_selection(), for_ordering(), for_grouping(), for_having(), for_limit(), for_join()],
        )
...
def template_dic_to_nl(template_dic, is_recursive_call=False):
    ...
    if template_dic["grouping"]:
        group_by_phrase = concat_phrases(
            "for each", " and ".join([str(t["template"]) for t in template_dic["grouping"]])
        )
        if template_dic["having"]:
            having_phrase = concat_phrases(
                "having", " and ".join([t["template"].nl_for_grouping for t in template_dic["having"]])
            )
    ...
    if template_dic["ordering"]:
        ordering_phrase = concat_phrases(
            "order by", " and ".join([str(t["template"]) for t in template_dic["ordering"]])
        )
