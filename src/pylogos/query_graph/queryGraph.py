...
class Node(metaclass=ABCMeta):
    ...
    def is_equivalent(node1, node2):
        return (
            ...
            and node1.query_block_idx == node2.query_block_idx
            and node1.nesting_level == node2.nesting_level
        )
...
class Function(Node):
    def __init__(self, aggregation_type, query_block_idx=0, nesting_level=0):
        super().__init__(
            aggregationTypeToName[aggregation_type], query_block_idx=query_block_idx, nesting_level=nesting_level
        )
        self.abbre = "F"
...
class Edge(metaclass=ABCMeta):
    ...
    def is_similar(edge1, edge2):
        try:
            return (
                ...
                and Node.is_similar(edge1.src, edge2.src)
                and Node.is_similar(edge1.dst, edge2.dst)
            )
        except:
            return False
    ...
    def is_equivalent(edge1, edge2):
        ...
        return (
            edge1.name == edge2.name
            and Node.is_equivalent(edge1.src, edge2.src)
            and Node.is_equivalent(edge1.dst, edge2.dst)
        )
...
class Query_graph(nx.DiGraph):
    ...
    def root(self):
        ...
        outer_block_relations = filter(
            lambda k: k.query_block_idx == 0 and k.nesting_level == 0 and type(k) == Relation, self.nodes._nodes
        )
    ...
    def set_join_directions(self, given_node=None):
        ...
        def is_for_join(n1, edge, n2):
            ...
            return type(edge["data"]) == Join or (
                type(edge["data"]) == Operation and type(n1) == Attribute and type(n2) == Attribute
            )
        ...
        for neighbor in self.neighbors(given_node):
            ...
            try:
                in_edge = self.edges[neighbor, given_node]
            except:
                in_edge = None
    ...
    def draw(self):
        labels = {
            key: f"{value.name}({value.nesting_level}.{value.query_block_idx})"
            for key, value in nx.get_node_attributes(self, "data").items()
        }
    ...
    def connect_join(self, relation1, r1_attribute, r2_attribute, relation2):
        ...
        self.bidirectional_connect(
            r1_attribute, Operation(OperatorType.Equal, r1_attribute, r2_attribute), r2_attribute
        )
    ...
    def connect_order(self, relation, attribute, is_asc=True):
        self.unidirectional_connect(
            relation,
            Ordering(OrderingType.Ascending if is_asc else OrderingType.Descending, relation, attribute),
            attribute,
        )
...
if __name__ == "__main__":
    ...
    ##
    print(
        "is isomorphic (custom matcher):",
        nx.algorithms.isomorphism.vf2userfunc.DiGraphMatcher(
            qg, qg2, node_match=node_matcher, edge_match=edge_matcher
        ).is_isomorphic(),
    )
    print(
        "subgraph is isomorphic:",
        nx.algorithms.isomorphism.vf2userfunc.DiGraphMatcher(
            qg, qg2, node_match=node_matcher, edge_match=edge_matcher
    ...
    subgraph = [
        i
        for i in nx.algorithms.isomorphism.vf2userfunc.DiGraphMatcher(
            qg, qg2, node_match=node_matcher, edge_match=edge_matcher
