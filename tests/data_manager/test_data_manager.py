...
EVQA_TEST_DIR = "./tests_data_manager_evqa/"
...
class Test_data_manager(unittest.TestCase):
    ...
    def _get_evqa_file_path(self, query_object: Any) -> str:
        return self._get_file_path(EVQA_TEST_DIR, query_object)

    def _test_loading_evqa(self) -> None:
        for query_class in self.example_query_classes:
            # Get file path and load data
            file_path = self._get_evqa_file_path(query_class())
            ...
            os.remove(file_path)
        print("Test loading EVQA Done!")

    def _test_saving_evqa(self) -> None:
        for query_class in self.example_query_classes:
            # Initiate the query object
            query_object = query_class()
            file_path = self._get_evqa_file_path(query_object)
            # Load the query object from the json file
            save_data(query_object.evqa, file_path)
            # Check if the file exists
            self.assertTrue(os.path.exists(file_path))
        print("Test loading EVQA Done!")

    # Test data structure for EVQA
    @test_utils.set_and_clean_test_dir(EVQA_TEST_DIR)
    def test_managing_eqvl(self) -> None:
        # Test saving
        self._test_saving_evqa()
        # Test loading
        self._test_loading_evqa()
